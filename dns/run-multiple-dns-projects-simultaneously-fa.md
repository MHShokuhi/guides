...
این تنظیمات رو پیدا کن و مقادیرش رو اینطوری تغییرشون بده:
...
خب ما با کمک dnsdist می‌تونیم بگیم هر ساب‌دامنه به کدوم پورت داخلی فرستاده بشه. پس این فایل رو باز کن:

```bash
sudo nano /etc/dnsdist/dnsdist.conf
```

کل محتویاتش رو پاک کن و اینا رو بگذار توش. در نظر داشته باشید که پورت‌ها و ساب‌دامنه‌ها باید مطابق چیزی باشه که از مرحله ۲ پیدا کرده یا از مراحل ۵ و ۶ تنظیم کرده بودیم. اون مقادیر داخل pool و در مقابلش PoolAction رو هم برای اینکه بعداً راحت‌تر باشی، برابر با مقدار tag برای ۳تا (هرچندتا) تونل dnstm قرار بده که مثلاً در نهایت اینطوری بشه:
...
- مقدار `pool` و `PoolAction` هر تونل باید یکی باشه که هدایت درست انجام بشه.
- یعنی در بخش `newServer` پورت‌های داخلی سرویس‌های فعلی رو وارد کن.
...
- در نهایت اگر تعداد تونل یا ابزار بیشتری داری، طبق همین الگو، در `addAction` و `newServer` اضافه کن و اگه کمتر هست خب پاک کن چیز بی‌استفاده نمونه.


## مرحله ۹: تایید تنظیمات dnsdist

برای تست فایل کانفیگی که نوشتیم برای اینکه ارور نداشته باشه این دستور رو بزن:
...
اگه هرچیزی آپدیت داد، ممکنه تنظیمات خودش رو برگردونه (theFeed این کار رو نمی‌کنه و اوکی هست) و بنابراین بعد از هر آپدیت مراحل ۳ و ۵ رو باید انجام بدید و در نهایت مرحله ۹ رو برای چک کردن اینکه همه چیز درست هست.
